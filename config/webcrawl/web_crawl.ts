...
import {
  ...
  buildContent,
  LIMIT,
  TELEMETRY_CRAWLER_EMPTY_DATA,
  TELEMETRY_RSS_LINK_HAS_EMPTY_DATA,
  TELEMETRY_HTML_ROOT_LINK_HAS_NO_LISTING,
  TELEMETRY_HTML_EXCEPTION_WHILE_FETCHING_STORY,
  TELEMETRY_CRAWLER_EXCEPTION_FOR_PUBLISHER
} from "../CONST";
...
export class WebCrawler {
  ...
  async crawl(
    list: Array<WebEntryPoint>,
    isTest: Boolean = false,
    name: string = null
  ) {
    try {
      await this.crawlInternal(list, isTest, name);
    } catch (e) {
      ex(e);
    }
  }
      ...
      try {
        ...
        stories = stories.map(storyDict => {
          ...
          if (storyDict["details"]) {
            storyDict["details"] = superCleanData(
              storyDict.url,
              storyDict["details"],
              web_entry
            );
          ...
          if (cont && validate(cont)) {
            ...
            if (web_entry.some_data_might_be_missing != true) {
              d(
                `[ERROR] $$$$$$$$$$ PLEASE CHECK THIS $$$$$$$$$$$$$ ${storyDict.url}`
              );
              d(cont);
              Analytics.hit_tracker({
                action: TELEMETRY_CRAWLER_EMPTY_DATA,
                link: storyDict.url
              });
            }
          }
        ...
        if (isTest) {
          if (stories.length == 0) {
            throw Error(
              "Please fix this now.: http://simplestore.dipankar.co.in/api/news_test/delete?_confirmstr=news_test"
            );
          }
          d(stories[0]);
        ...
        if (stories.length > 0) {
          await saveToDB(stories);
            ...
            Analytics.hit_tracker({'action':TELEMETRY_CRAWLER_EXCEPTION_FOR_PUBLISHER, 'name':web_entry.name})
            ex(e);
      }
    }
  }
  ...
  async processRssFeed(
    web_entry: WebEntryPoint,
    isTest: Boolean = false
  ): Promise<Array<any>> {
    let storyList = [];
    for (var link of web_entry.links) {
      let list = await web_entry.rsstype.read(link.url, {
        stream: link.stream
      });
      if (list.length == 0) {
        Analytics.hit_tracker({
          action: TELEMETRY_RSS_LINK_HAS_EMPTY_DATA,
          url: link.url
        });
      }
      for (var l of list) {
  ...
  async processWebFeed(
    ...
    for (var weblink of web_entry.links) {
      ...
      let parseResult = await findAllData(
        weblink.url,
        [
          {
            name: "urls",
            selector: list_selector,
            type: WebElementType.URL_LIST
          }
        ],
        $
      );
    ...
    for (var link of top_urls) {
      try {
        ...
        Analytics.hit_tracker({
          action: TELEMETRY_HTML_EXCEPTION_WHILE_FETCHING_STORY,
          link: link.url
        });
  ...
  addExtra(
    storyDict: import("../utils/types").StringAnyMap,
    web_entry: WebEntryPoint
  ) {
